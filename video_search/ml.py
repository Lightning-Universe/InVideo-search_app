...
class VideoProcessor:
    def __init__(self):
        ...
        model, preprocess = openai_clip.load("ViT-B/32", device=self.device)
        self.model = model
    ...
    def search_video(self, video_id: str, search_query: str, results_count: int):
        print(f"Video search started ({video_id}, {search_query})")
        ...
        with torch.no_grad():
            text_features = self.model.encode_text(openai_clip.tokenize(search_query).to(self.device))
        ...
        print(f"Video search finished ({video_id}, {search_query})")
